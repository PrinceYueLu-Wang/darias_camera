...
public:
    ...
    {   
        ...
        ROS_INFO_STREAM("pcd camera num " << pcd_arm->size());
        ...
        end_ = ros::WallTime::now();
        execution_time = (end_ - start_).toNSec() * 1e-6;
        ROS_INFO_STREAM("Exectution c time (ms): " << execution_time);
        ...
        ROS_INFO_STREAM("octree num " << cube_num);

        }

        end_ = ros::WallTime::now();
        execution_time = (end_ - start_).toNSec() * 1e-6;
        ROS_INFO_STREAM("Exectution b time (ms): " << execution_time);

        
        ROS_INFO_STREAM("pcd num " << pcd_cameraPcd_filtered->size());
    }
